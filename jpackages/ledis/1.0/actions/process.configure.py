def main(j,jp):
    ...
    pd=j.tools.startupmanager.addProcess(\
        name=jp.name,\
        cmd="source env.sh;./ledis-server -config=ledis.conf", \
        args="",\
        env={},\
        numprocesses=1,\
        priority=100,\
        shell=False,\
        workingdir='/opt/ledis/',\
        jpackage=jp,\
        domain=jp.domain,\
        ports=[6380,11181],\
        autostart=True,\
        ...
        upstart=False,\
        stats=False,\
        processfilterstr="ledis-server")#what to look for when doing ps ax to find the process
